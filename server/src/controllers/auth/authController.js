  ...
  try {
    ...
    if (user.role === "ADMIN" || user.role === "SUPER_ADMIN") {
      ...
      import("../../utils/mailer.js").then(({ sendEmail }) => {
        sendEmail({
          to: user.email,
          subject: "Admin Login OTP - Valuexpert",
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2>Your Admin Login OTP</h2>
              <p>Your one-time password to access the Valuexpert Admin Dashboard is:</p>
...
export async function requestAdminPasswordReset(req, res) {
  try {
    ...
    import("../../utils/mailer.js").then(({ sendEmail }) => {
      sendEmail({
        to: user.email,
        subject: "Password Reset OTP - Valuexpert",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2>Reset Your Password</h2>
            <p>Use the one-time password below to reset your Valuexpert password:</p>
            <h1 style="background: #f4f4f5; padding: 10px 20px; letter-spacing: 5px; text-align: center;">${otp}</h1>
            <p>This OTP will expire in 10 minutes.</p>
