...
export default function AboutUsPage() {
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* Content Section */}
      <section className="relative py-16 lg:py-8 md:py-24 bg-slate-50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-[0_8px_40px_rgb(0,0,0,0.06)] border border-slate-100 transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3),0_0_20px_rgba(210,144,82,0.1)] hover:border-gold/50">
              ...
              <div className="space-y-4">
                {democaHighlights.map((item, i) => (
                  <div key={i} className="flex items-start gap-4">
                    <div className="mt-1 text-gold"><CheckCircle size={20} /></div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-3 gap-8">
            {pillars.map((pillar, i) => (
              <article key={pillar.title} className="group flex flex-col h-full p-8 rounded-none bg-white border border-slate-200 shadow-sm hover:shadow-xl hover:border-primary/40 hover:-translate-y-1 transition-all duration-300">
                <div className="w-14 h-14 rounded-none bg-slate-50 text-slate-600 flex items-center justify-center mb-6 group-hover:bg-gold group-hover:text-white transition-colors duration-300">
                  {pillar.icon}
                </div>
      ...
      <section className="relative py-16 lg:py-8 md:py-24 bg-slate-50 text-slate-900 overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <span className="inline-block rounded-full bg-white border border-slate-200 shadow-sm px-4 py-1.5 text-xs font-black uppercase tracking-widest text-gold mb-4">Popular Services</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
            {featuredServices.slice(0, 6).map((service, index) => (
              <Link key={service.title} href={service.href} className="group flex flex-col h-full p-8 rounded-none bg-white border border-slate-200 shadow-sm hover:shadow-xl hover:border-primary/40 transition-all duration-300 hover:-translate-y-1">
                <span className="text-gold text-sm font-black uppercase tracking-widest mb-3 block">Service {String(index + 1).padStart(2, "0")}</span>
                ...
                <div className="mt-auto pt-2">
                  <span className="inline-flex items-center gap-2 text-sm font-bold text-gold group-hover:text-gold-600">
