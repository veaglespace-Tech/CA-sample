...
import { ChevronRight, Calculator, PieChart, Home } from "lucide-react";
...
export default function EmiCalculator() {
  ...
  const calculateEmi = () => {
    ...
    const r = Math.max(0, parseFloat(interestRate) || 0) / 12 / 100;
    const n = Math.floor(Math.max(0, parseFloat(tenureYears) || 0) * 12);

    if (P === 0 || n === 0) return { emi: 0, totalInterest: 0, totalPayment: 0 };
    if (r === 0) return { emi: (P / n).toFixed(0), totalInterest: 0, totalPayment: P.toFixed(0) };
      ...
      <section className="relative overflow-hidden bg-white z-20 pt-24 pb-16 lg:pt-32 lg:pb-24 border-b border-slate-200/60">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight mb-6 animate-fade-in-up" style={{ animationDelay: "100ms" }}>
            EMI <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-300 to-amber-300">Calculator</span>
          </h1>
          <p className="max-w-2xl mx-auto text-lg text-slate-600 font-medium leading-relaxed animate-fade-in-up" style={{ animationDelay: "200ms" }}>
            Calculate your Equated Monthly Instalment (EMI) for Home, Car or Personal Loans instantly.
          </p>
        </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
              <div className="flex items-center gap-3 mb-8 pb-4 border-b border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-500">Enter your loan amount, interest, and tenure.</p>
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-black uppercase tracking-widest text-slate-500 mb-4">Loan Tenure (Years)</label>
                  <div className="relative">
                    <input 
                      type="number" 
                      className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl outline-none transition-all text-xl font-black text-slate-900 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10" 
            ...
            <div className="relative rounded-3xl bg-gradient-to-br from-blue-900 to-indigo-900 p-8 text-white shadow-2xl overflow-hidden h-fit">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-blue-400 mb-6">
                  <PieChart size={14} /> Repayment Summary
                </div>
                ...
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-5 rounded-2xl shadow-lg border border-blue-500/50 relative overflow-hidden mt-6">
                  ...
                  <strong className="text-2xl font-black text-white relative z-10">
                    ₹ {parseInt(results.totalPayment).toLocaleString()}
                  </strong>
                </div>
