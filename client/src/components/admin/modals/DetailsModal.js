...
export default function DetailsModal({ currentItem, onClose, onSwitchItem, onSendMessage, allUsers, allRegistrations = [], allLeads = [], setEmailingContact }) {
  ...
  const handleSend = async () => {
    ...
    if (!finalUserId) {
      alert("Cannot send message: This record is not linked to a registered user account. Please ensure the client has registered on Your Company Name with the same email address.");
      return;
    }
