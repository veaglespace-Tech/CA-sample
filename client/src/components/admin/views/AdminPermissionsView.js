...
function Toggle({ enabled, onChange }) {
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1 ${enabled ? "bg-indigo-600" : "bg-slate-200"}`}
    >
...
function ModuleSection({ moduleKey, schema, permissions, onChange }) {
  ...
  return (
    <div className={`rounded-2xl border transition-all duration-150 overflow-hidden ${open ? `${c.border} border` : "border-slate-100"}`}>
      {/* Header row */}
      <div
      ...
      >
        ...
        <div className="w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden flex-shrink-0">
          <div
            className={`h-full rounded-full transition-all ${allOn ? "bg-emerald-500" : enabledCount === 0 ? "bg-rose-400" : "bg-indigo-500"}`}
        ...
        <div className="border-t border-slate-100 px-4 py-4 bg-white">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-1.5">
              <button onClick={grantAll} className="px-2 py-1 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-black hover:bg-emerald-100 transition-colors">All On</button>
              <button onClick={revokeAll} className="px-2 py-1 rounded-lg bg-rose-50 border border-rose-200 text-rose-700 text-[10px] font-black hover:bg-rose-100 transition-colors">All Off</button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {allKeys.map(actionKey => {
              ...
              return (
                <div
                  key={actionKey}
                  className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${isEnabled ? `${rs.badge} border` : "bg-slate-50 border-slate-100"}`}
                  onClick={() => toggleAction(actionKey, !isEnabled)}
                >
                  <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 ${isEnabled ? "bg-white/70" : "bg-slate-100"}`}>
                    <ActionIcon size={12} className={isEnabled ? (am.risk === "danger" ? "text-rose-500" : am.risk === "moderate" ? "text-amber-500" : "text-emerald-500") : "text-slate-300"} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-black ${isEnabled ? "text-slate-900" : "text-slate-400"}`}>{am.label}</p>
                    <p className={`text-[10px] font-medium mt-0.5 ${isEnabled ? "text-slate-500" : "text-slate-300"}`}>
                      {actions[actionKey]}
                    </p>
                  </div>
...
function AdminCard({ admin, schema, isSaving, onSave }) {
  ...
  return (
    <div className={`bg-white rounded-2xl border overflow-hidden transition-all duration-200 ${dirty ? "border-indigo-300 shadow-lg shadow-indigo-100" : "border-slate-100 shadow-sm"}`}>
      {/* Header */}
      <div className="p-5 flex items-center gap-4">
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-black text-lg shadow-inner flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5">
            <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden max-w-[120px]">
              <div
                className={`h-full rounded-full transition-all duration-500 ${pct === 100 ? "bg-emerald-500" : pct === 0 ? "bg-rose-400" : "bg-indigo-500"}`}
                style={{ width: `${pct}%` }}
              />
            </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {dirty && (
            <button onClick={handleSave} disabled={isSaving}
              className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black transition-all shadow-md shadow-indigo-200 disabled:opacity-50">
              <Save size={12} /> Save
            </button>
          )}
        ...
        <div className="border-t border-slate-100 px-5 py-5">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button onClick={grantAll} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 text-[11px] font-bold hover:bg-emerald-100 transition-colors">
                <Unlock size={10} /> Grant All
              </button>
              <button onClick={revokeAll} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-rose-50 border border-rose-200 text-rose-700 text-[11px] font-bold hover:bg-rose-100 transition-colors">
          ...
          {dirty && (
            <div className="mt-5 pt-4 border-t border-slate-100 flex justify-end">
              <button onClick={handleSave} disabled={isSaving}
                className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black transition-all shadow-md shadow-indigo-200 disabled:opacity-50">
...
export default function AdminPermissionsView() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Header */}
      <div className="bg-gradient-to-tr from-slate-900 via-indigo-950 to-violet-950 text-white rounded-[2.5rem] p-8 shadow-xl relative border border-slate-800 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl md:text-4xl font-black tracking-tight leading-none text-white">Staff Permissions</h1>
          ...
          <button onClick={refetch} className="flex items-center gap-2 px-4 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-xs font-bold text-white transition-all">
      ...
      {saved && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 animate-in fade-in duration-300">
      ...
      {!isLoading && !isError && admins.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Total Admins",  value: admins.length, icon: UserCog, color: "text-indigo-600", bg: "bg-indigo-50" },
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-sm font-semibold text-slate-400">Loading admin accounts...</p>
        </div>
      )}
      ...
      {isError && (
        <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
          ...
          <div>
            <p className="font-black text-slate-800">Failed to load permissions</p>
            <p className="text-sm text-slate-400 mt-1">Could not connect to the server.</p>
          </div>
          <button onClick={refetch} className="px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold">Try Again</button>
        </div>
      )}
      ...
      {!isLoading && !isError && admins.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
          ...
          <div>
            <p className="font-black text-slate-800">No Admin accounts found</p>
            <p className="text-sm text-slate-400 mt-1">Go to <span className="font-bold text-indigo-600">User Management</span> and create an account with the Admin role first.</p>
          </div>
        </div>
      )}
      ...
      {!isLoading && !isError && admins.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <p className="text-xs font-semibold text-slate-400">Expand a card to configure permissions</p>
          </div>
          {admins.map(admin => (
            <AdminCard
              key={admin.id}
