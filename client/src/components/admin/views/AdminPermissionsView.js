...
const COLOR_MAP = {
  ...
  slate:   { bg: "bg-navy-light",  text: "text-slate-300",   border: "border-slate-700"   },
...
function Toggle({ enabled, onChange }) {
  return (
    ...
    >
      <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-navy shadow transition duration-200 ${enabled ? "translate-x-4" : "translate-x-0"}`} />
    </button>
  );
}
...
function ModuleSection({ moduleKey, schema, permissions, onChange }) {
  ...
  return (
    <div className={`rounded-none border transition-all duration-150 overflow-hidden ${open ? `${c.border} border` : "border-slate-800"}`}>
      {/* Header row */}
      <div
        className={`flex items-center gap-3 px-4 py-3.5 cursor-pointer select-none ${open ? c.bg : "bg-navy hover:bg-navy-light"}`}
        onClick={() => setOpen(p => !p)}
      >
        ...
        {/* Mini progress */}
        <div className="w-20 h-1.5 bg-navy-light rounded-full overflow-hidden flex-shrink-0">
          <div
      ...
      {/* Expanded actions */}
      {open && (
        <div className="border-t border-slate-800 px-4 py-4 bg-navy">
          ...
          <div className="space-y-2">
            {allKeys.map(actionKey => {
              ...
              return (
                <div
                  key={actionKey}
                  className={`flex items-center gap-3 p-3 rounded-sm border cursor-pointer transition-all ${isEnabled ? `${rs.badge} border` : "bg-navy-light border-slate-800"}`}
                  onClick={() => toggleAction(actionKey, !isEnabled)}
                >
                  <div className={`w-6 h-6 rounded-sm flex items-center justify-center flex-shrink-0 ${isEnabled ? "bg-navy/70" : "bg-navy-light"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-black ${isEnabled ? "text-white" : "text-slate-400"}`}>{am.label}</p>
                    <p className={`text-[10px] font-medium mt-0.5 ${isEnabled ? "text-slate-400" : "text-slate-300"}`}>
                      {actions[actionKey]}
                    </p>
                  </div>
...
function AdminCard({ admin, schema, isSaving, onSave }) {
  ...
  return (
    <div className={`bg-navy rounded-none border overflow-hidden transition-all duration-200 ${dirty ? "border-indigo-300 shadow-lg shadow-indigo-100" : "border-slate-800 shadow-sm"}`}>
      {/* Header */}
      <div className="p-5 flex items-center gap-4">
        <div className="w-12 h-12 rounded-sm bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-black text-lg shadow-inner flex-shrink-0">
          {admin.name?.charAt(0)?.toUpperCase() || "A"}
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-black text-white text-sm">{admin.name}</p>
          <p className="text-xs text-slate-400 truncate">{admin.email}</p>
          <div className="flex items-center gap-3 mt-1.5">
            <div className="flex-1 h-1.5 bg-navy-light rounded-full overflow-hidden max-w-[120px]">
              <div
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button onClick={() => setExpanded(p => !p)}
            className="flex items-center gap-1 px-3 py-2 bg-navy-light hover:bg-slate-200 text-slate-300 rounded-sm text-xs font-bold transition-all">
      ...
      {expanded && (
        <div className="border-t border-slate-800 px-5 py-5">
          ...
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Module Permissions</p>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-gradient-to-tr from-slate-900 via-indigo-950 to-violet-950 text-white rounded-[2.5rem] p-8 shadow-xl relative border border-slate-800 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <button onClick={refetch} className="flex items-center gap-2 px-4 py-2.5 bg-navy/10 hover:bg-navy/20 border border-white/20 rounded-sm text-xs font-bold text-white transition-all">
      ...
      {!isLoading && !isError && admins.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={label} className="bg-navy rounded-none border border-slate-800 shadow-sm p-5 flex items-center gap-4 transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3),0_0_20px_rgba(210,144,82,0.1)] hover:border-gold/50">
      ...
      {isError && (
        <div className="flex flex-col items-center justify-center py-8 md:py-20 gap-4 text-center">
          ...
          <div>
            <p className="font-black text-white">Failed to load permissions</p>
            <p className="text-sm text-slate-400 mt-1">Could not connect to the server.</p>
          </div>
          <button onClick={refetch} className="px-4 py-2 bg-slate-900 text-white rounded-sm text-xs font-bold">Try Again</button>
        </div>
      )}
      ...
      {!isLoading && !isError && admins.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 md:py-20 gap-4 text-center">
          <div className="w-14 h-14 bg-navy-light rounded-full flex items-center justify-center">
            <UserCog size={24} className="text-slate-400" />
          </div>
          <div>
            <p className="font-black text-white">No Admin accounts found</p>
            <p className="text-sm text-slate-400 mt-1">Go to <span className="font-bold text-gold">User Management</span> and create an account with the Admin role first.</p>
          </div>
        </div>
      )}
