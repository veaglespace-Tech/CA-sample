...
export default function PlansView({ serviceCategories = [], users = [], onAdd, onEdit, onAssignPlan, onDelete }) {
  ...
  const normalizedServiceSlug = serviceFilter === "ALL"
    ? ""
  ...
  const directServices = activeCategory?.services || [];
  const targetServices = selectedSubcategory
  ...
  const summaryLabel = meta.total === 0
    ? "No plans found"
    : `Showing ${(meta.page - 1) * PAGE_SIZE + 1}-${Math.min(meta.page * PAGE_SIZE, meta.total)} of ${meta.total} plans`;

  const categoryName = activeCategory?.name || null;
  const subcategoryName = selectedSubcategory?.name || null;
  const selectedServiceName = targetServices.find((service) => service.slug === normalizedServiceSlug)?.name || null;
          ...
          <table className="w-full min-w-[820px]">
            <thead>
              <tr className="border-b border-slate-800 bg-navy-light/80">
                ...
                <th className="px-6 py-4 text-left text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">Tag</th>
                <th className="px-6 py-4 text-right text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">Actions</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/45 px-4 py-6 backdrop-blur-sm">
          ...
          >
            ...
            <div className="space-y-5 px-6 py-5">
              ...
              <label className="block space-y-2">
                <span className="text-[11px] font-black uppercase tracking-[0.16em] text-slate-400">Internal Note</span>
                <textarea
                  value={assignNote}
