        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 bg-indigo-100/70 rounded-full border border-indigo-200/50">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider text-indigo-700">Compliance Hub</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight leading-none text-slate-800">Document Workspace</h1>
        ...
        <div className="grid grid-cols-1 gap-6">
          {allDocRequirements.map((reqDoc) => {
            ...
            return (
              <div 
                key={reqDoc.type}
                className="bg-white/90 backdrop-blur-xl border border-slate-200/60 shadow-xl rounded-[2.5rem] p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:shadow-2xl hover:border-slate-300/80 transition-all duration-300 relative overflow-hidden group"
              >
                {/* Subtle gradient background mesh */}
                <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-500/5 rounded-full blur-3xl group-hover:bg-indigo-500/10 transition-colors duration-500 -z-10 pointer-events-none"></div>
                <div className="space-y-2 max-w-md">
                  <div className="flex items-center gap-3">
                    <h3 className="font-black text-slate-900 text-xl tracking-tight">{reqDoc.type}</h3>
                    {reqDoc.required && (
                      <span className="text-[9px] font-black uppercase tracking-wider px-2.5 py-1 bg-rose-50 border border-rose-100/50 text-rose-600 rounded-lg">
                        Required
                      </span>
                    )}
                  </div>
                ...
                {matchedDoc ? (
                  ...
                  <div className="w-full md:w-auto flex flex-col sm:flex-row items-start sm:items-center gap-5 bg-gradient-to-br from-slate-50/80 to-white border border-slate-100/60 p-5 rounded-[2rem] self-stretch md:self-auto justify-between sm:justify-start hover:border-indigo-100/60 hover:shadow-[0_8px_20px_-6px_rgba(0,0,0,0.05)] transition-all">
                    <div className="space-y-1">
                      <div className="text-sm font-black text-slate-800 truncate max-w-[200px]" title={matchedDoc.fileName}>
                        {matchedDoc.fileName}
                      </div>
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-end sm:justify-start pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-200/50 mt-1 sm:mt-0">
                      ...
                      <button 
                        ...
                        className="btn btn-sm bg-rose-50 border-none hover:bg-rose-100 text-rose-600 rounded-xl h-10 w-10 px-0 transition-all duration-300 shadow-sm flex items-center justify-center"
                        title="Delete file"
                      >
                        <Trash2 size={18} />
                      </button>
                  ...
                  <div className="w-full md:w-auto self-stretch md:self-auto shrink-0">
                    <label className="relative flex flex-col items-center justify-center border-[1.5px] border-dashed border-slate-300 hover:border-indigo-400 bg-slate-50 hover:bg-indigo-50/40 rounded-3xl p-6 cursor-pointer text-center group transition-all duration-300 w-full md:min-w-[280px] shadow-sm hover:shadow-md">
                      <input 
                        type="file"
                      ...
                      {uploadingDocType === reqDoc.type ? (
                        <div className="flex flex-col items-center gap-3 py-2">
                          ...
                          <span className="text-[11px] font-black text-indigo-600 uppercase tracking-widest">Uploading...</span>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center">
                          <div className="p-3 bg-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-slate-100 rounded-full mb-3 group-hover:scale-110 transition-transform duration-300">
                            <Upload size={20} className="text-slate-400 group-hover:text-indigo-500 transition-colors" />
                          </div>
                          <div className="text-[11px] font-black uppercase text-slate-500 group-hover:text-indigo-600 tracking-widest transition-colors">
                            Click to Upload
                          </div>
