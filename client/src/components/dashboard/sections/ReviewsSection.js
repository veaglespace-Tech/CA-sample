  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="w-full flex items-center justify-between gap-2 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:border-indigo-300 focus:outline-none focus:border-indigo-400 transition-all"
      >
      ...
      {open && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-2xl shadow-slate-900/10 overflow-hidden">
          <div className="p-2 border-b border-slate-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-300"
                autoFocus
              />
            </div>
          </div>
          <div className="max-h-56 overflow-y-auto">
            ...
            {filtered.map((s) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-sm hover:bg-indigo-50 transition-colors text-left ${value === s.slug ? "bg-indigo-50 text-indigo-700 font-bold" : "text-slate-700 font-medium"}`}
              >
                <Building2 size={14} className="shrink-0" />
                <span>{s.label}</span>
                {value === s.slug && <Check size={14} className="ml-auto text-indigo-600" />}
              </button>
            ))}
            {filtered.length === 0 && (
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          onClick={openCreate}
          className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 hover:-translate-y-0.5 transition-all duration-300"
        >
          <Plus size={18} />
          Add Review
        </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
          />
        </div>
        ...
        <select
          ...
          className="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
        >
      ...
      <div className="flex items-center gap-3 text-sm text-slate-500 font-medium px-1">
        <span>
          Showing <strong className="text-slate-800">{reviews.length}</strong> reviews
          {selectedServiceLabel && <span className="ml-1 text-indigo-600">for &quot;{selectedServiceLabel}&quot;</span>}
      ...
      {isLoading || isFetching ? (
        ...
        <div className="bg-white rounded-3xl border-2 border-dashed border-slate-200 p-16 text-center">
          ...
          <h3 className="text-lg font-black text-slate-700">No Reviews Found</h3>
          <p className="text-sm text-slate-400 font-medium mt-1">Add your first review to get started.</p>
          <button
            onClick={openCreate}
            className="mt-5 inline-flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:-translate-y-0.5 transition-all"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {reviews.map((review) => (
            <div
              key={review.id}
              className="group bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md hover:border-slate-200 transition-all duration-300 flex flex-col h-full"
            >
              {/* Stars + Status */}
              ...
              {review.serviceSlug && (
                <div className="flex items-center gap-1.5 mb-3">
                  ...
                  <span className="text-[11px] font-bold text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded-full truncate max-w-full">
              ...
              <div className="flex items-center justify-between border-t border-slate-50 pt-3 mt-auto">
                ...
                <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  <button
                    ...
                    className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 transition-all"
                  >
                    <Pencil size={14} />
                  </button>
                  <button
                    onClick={() => setDeleteConfirm(review)}
                    className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-500 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-all"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
            ...
            <form onSubmit={handleSave} className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100 transition-all resize-none"
                  required
                />
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      required
                      className="w-full pl-9 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100 transition-all"
                    />
                  </div>
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                />
                {form.serviceSlug && (
                  <p className="text-xs text-indigo-600 font-bold flex items-center gap-1.5 mt-1">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-700 focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100 transition-all"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 text-white text-sm font-black shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/35 hover:-translate-y-0.5 disabled:opacity-60 disabled:pointer-events-none transition-all duration-300"
                >
                  {isSaving ? (
        ...
        <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 text-center animate-in zoom-in-95 fade-in duration-300">
            ...
            <p className="text-sm text-slate-500 font-medium mb-6">
              Review by <strong className="text-slate-700">&quot;{deleteConfirm.name}&quot;</strong> will be permanently removed.
            </p>
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isDeleting}
                className="flex-1 py-3 rounded-xl bg-rose-600 text-white text-sm font-black hover:bg-rose-700 disabled:opacity-50 transition-colors"
              >
