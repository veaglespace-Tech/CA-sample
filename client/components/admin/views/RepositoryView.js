...
export default function RepositoryView({ allUsers, onSendMessage, permissions = {} }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <select 
            ...
            className="px-4 py-2 bg-white border border-slate-200 rounded-sm text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all shadow-sm"
          >
          ...
          <button
            ...
            className={`btn rounded-none font-black gap-2 transition-all shadow-sm ${
              showUploadForm
                ? "btn-ghost border border-slate-200 text-slate-600 hover:bg-slate-50"
                : "btn-primary px-6"
            }`}
          >
            {showUploadForm ? (
      ...
      {/* Upload / Edit Form */}
      {showUploadForm && (
        <div className="bg-white border border-slate-100 rounded-[2rem] shadow-md overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300 transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3),0_0_20px_rgba(210,144,82,0.1)] hover:border-gold/50">
          ...
          <form onSubmit={handleUpload} className="p-8 transition-all duration-400 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.2)] hover:border-gold/30">
            <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-sm text-sm font-semibold text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-sm text-sm font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                >
                  <option value="TEMPLATE">Template</option>
                  <option value="COMPLIANCE">Compliance Form</option>
                  <option value="GUIDE">User Guide</option>
              ...
              <div className="md:col-span-2 space-y-1.5">
                <label className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider text-slate-500">
                  <AlignLeft size={12} /> Description <span className="text-slate-500 normal-case font-medium">(optional)</span>
                </label>
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-sm text-sm font-semibold text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all resize-none"
                />
              </div>
              ...
              <div className="md:col-span-2 space-y-1.5">
                <label className="flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider text-slate-500">
                  <Paperclip size={12} /> File {editingDoc && <span className="text-slate-500 normal-case font-medium">(leave empty to keep current)</span>}
                </label>
                <label className="flex items-center gap-4 px-5 py-4 bg-slate-50 border-2 border-dashed border-slate-200 hover:border-primary/40 hover:bg-gold/[0.02] rounded-sm cursor-pointer transition-all group">
                  <div className="p-2.5 bg-white border border-slate-200 rounded-sm group-hover:border-primary/30 transition-all shrink-0 transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3),0_0_20px_rgba(210,144,82,0.1)] hover:border-gold/50">
                    <Upload size={18} className="text-slate-500 group-hover:text-gold transition-colors" />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">PDF, DOCX, XLSX, PNG supported</p>
                  </div>
                  <input
                    type="file"
                    required={!editingDoc}
            ...
            <div className="flex justify-end gap-3 mt-6 pt-5 border-t border-slate-100">
              <button
                ...
                className="btn btn-ghost rounded-sm px-6 font-bold text-slate-600 hover:bg-slate-50 border border-slate-200"
              >
                Cancel
              </button>
              <button
      ...
      {sharingDoc && (
        <dialog className="modal modal-open">
          <div className="modal-box w-11/12 max-w-md rounded-[2rem] p-0 overflow-hidden bg-white shadow-2xl border border-slate-100 flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200 transition-all duration-400 hover:-translate-y-1.5 hover:shadow-[0_15px_40px_-10px_rgba(0,0,0,0.3),0_0_20px_rgba(210,144,82,0.1)] hover:border-gold/50">
            ...
            <div className="bg-slate-900 px-6 py-5 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900 tracking-tight">Share Document</h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <p className="text-[11px] text-slate-500 font-semibold truncate max-w-[150px]">{sharingDoc.fileName}</p>
                  </div>
                </div>
              </div>
              <button
                onClick={() => { setSharingDoc(null); setSearchTerm(""); }}
                className="btn btn-circle btn-sm border-none bg-slate-800 hover:bg-slate-700 text-slate-600 font-bold"
              >
                ✕
              </button>
            </div>
            ...
            <div className="px-5 pt-5 pb-3 shrink-0">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-sm text-sm font-semibold text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-2">
              {filteredUsers.length > 0 ? (
                filteredUsers.map((user) => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between px-4 py-3 bg-slate-50 border border-slate-100 rounded-sm hover:border-primary/20 hover:bg-gold/[0.015] transition-all"
                  >
                    <div>
